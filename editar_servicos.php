...
if (isset($_POST['editar_servico'])) {
    ...
    $sql_code = "UPDATE servicos SET servico = '$servico', preco = '$preco' WHERE idServico = '$idServico'";

    if ($mysqli->query($sql_code)) {
        echo "<div class='alert alert-success'>Serviço atualizado com sucesso!</div>";
    } else {
        die("Erro ao atualizar serviço: " . $mysqli->error);
    }
}
...
if (isset($_POST['servico_id'])) {
    $idServico = $_POST['servico_id'];
    $sql_code = "SELECT * FROM servicos WHERE idServico = '$idServico'";
    $sql_query = $mysqli->query($sql_code) or die($mysqli->error);

    if ($sql_query->num_rows > 0) {
        $servicoSelecionado = $sql_query->fetch_assoc();
    } else {
                ...
                <div class="container-fluid">
                    ...
                    <form method="POST" action="">
                        <div class="form-group">
                            ...
                            <select id="servico_id" name="servico_id" class="form-control" onchange="this.form.submit()"
                                required>
                                <option value="">Selecione um serviço</option>
                                <?php foreach ($servicos as $servico): ?>
                                    <option value="<?php echo htmlspecialchars($servico['idServico']); ?>"
                                        <?php echo (isset($servicoSelecionado) && $servicoSelecionado['idServico'] == $servico['idServico']) ? 'selected' : ''; ?>>
                                        <?php echo htmlspecialchars($servico['servico']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <?php if ($servicoSelecionado): ?>
                        <form method="POST" action="">
                            <input type="hidden" name="idServico"
                                value="<?php echo htmlspecialchars($servicoSelecionado['idServico']); ?>">
                            <div class="form-group">
                                <label for="servico">Serviço:</label>
                                <input type="text" class="form-control" id="servico" name="servico"
                                    value="<?php echo htmlspecialchars($servicoSelecionado['servico']); ?>" required>
                            </div>
                            <div class="form-group">
                                <label for="preco">Preço:</label>
                                <input type="text" class="form-control" id="preco" name="preco"
                                    value="<?php echo htmlspecialchars($servicoSelecionado['preco']); ?>" required>
                            </div>
                            <button type="submit" name="editar_servico" class="btn btn-primary">Atualizar Serviço</button>
                        </form>
                    <?php endif; ?>
                </div>
