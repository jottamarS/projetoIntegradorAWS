...
if (isset($_POST['aprovar_agendamento'])) {

    $idAgendamento = $_POST['idAgendamento'];


    $sql_code = "UPDATE agendamento SET status = 'Agendado' WHERE idAgendamento = ?";
    ...
    if ($stmt = $mysqli->prepare($sql_code)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "Erro ao aprovar o agendamento: " . $stmt->error;
        }
        $stmt->close();
    } else {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="container-fluid">
                        <H2 style="text-align: center;">Aprovação de serviços.</H2>
                        <?php
                        $sql_code = "SELECT * FROM agendamento WHERE status = 'aguardando aprovação'";
                        ...
                        if ($sql_query->num_rows > 0) {
                            ...
                            while ($agendamento = $sql_query->fetch_assoc()) {
                                ...
                                echo "<td>
                            <form method='POST' action='aprovar_agendamentos.php'>
                                <input type='hidden' name='idAgendamento' value='" . htmlspecialchars($agendamento['idAgendamento']) . "'>
                                <button type='submit' name='aprovar_agendamento' class='btn btn-success'>Aprovar</button>
                            </form>
                            </td>";
                                echo "</tr>";
                            }
                            ...
                            echo '<div class="alert alert-info" style="text-align: center;">Nenhum agendamento aguardando aprovação.</div>';
                        }
                        ?>
                    </div>
                </div>
